...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "bulk", "data", "programs", "tests", "eg"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "README.md",
...
def test_only_egypt_namespace_present() -> None:
    """Egypt scope: the only jurisdiction directory is eg/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"eg"}, f"unexpected jurisdiction dirs: {names - {'eg'}}"


def test_eg_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "eg" / marker).is_dir(), f"missing eg/{marker}"
...
def test_oracle_index_is_eg_scoped() -> None:
    payload = json.loads((ROOT / "data/oracles/oracle-index.json").read_text())
    assert payload["jurisdiction"] == "eg"


def test_source_map_is_eg_scoped() -> None:
    payload = json.loads(
